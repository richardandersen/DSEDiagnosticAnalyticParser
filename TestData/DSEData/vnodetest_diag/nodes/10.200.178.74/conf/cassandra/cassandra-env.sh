...
# starting cassandra.
   ...
   JVM_OPTS="$JVM_OPTS -XX:+UseG1GC"  # set by Opscenter Provisioning
   
   JVM_OPTS="$JVM_OPTS -XX:G1RSetUpdatingPauseTimePercent=5"  # set by Opscenter Provisioning
   
   JVM_OPTS="$JVM_OPTS -XX:MaxGCPauseMillis=500"  # set by Opscenter Provisioning
...
# We only set -Xms and -Xmx if they were not defined previously
# If defined, both Xmx and Xms should be defined together.
if [ $DEFINED_XMX -ne 0 ] && [ $DEFINED_XMS -ne 0 ]; then
     JVM_OPTS="$JVM_OPTS -Xms${MAX_HEAP_SIZE}"
     JVM_OPTS="$JVM_OPTS -Xmx${MAX_HEAP_SIZE}"
elif [ $DEFINED_XMX -ne 0 ] || [ $DEFINED_XMS -ne 0 ]; then
     echo "Please set or unset -Xmx and -Xms flags in pairs."
     exit 1
fi
...
if [ $DEFINED_XMN -eq 0 ] && [ $DEFINED_XMX -ne 0 ]; then
    echo "Please set or unset -Xmx and -Xmn flags in pairs."
    exit 1
