...
# Have the JVM do less reference set updating during STW, instead
...
# The JVM maximum is 8 PGC threads and 1/4 of taht for ConcGC
# Machines with > 10 cores may need additional threads. Increase to <= full cores.
JVM_OPTS="$JVM_OPTS -XX:ParallelGCThreads=12"
JVM_OPTS="$JVM_OPTS -XX:ConcGCThreads=3"

# Main G1GC tunable: lowering the pause target will lower throughput and vice-versa
# 200ms is the JVM default and lowest viable setting
# 1000ms increases throughput. Keep it smaller than the timeouts in cassandra.yaml
JVM_OPTS="$JVM_OPTS -XX:MaxGCPauseMillis=500"

# Save CPU time on large (>= 16GB) heaps by delaying region scanning until the heap
# is 70% full. The default in Hotspot 8u40 is 40%
...
# http://wiki.apache.org/solr/ShawnHeisey and latest DSE recommendation
JVM_OPTS="$JVM_OPTS -XX:+ParallelRefProcEnabled"
# To better handle humongous objects in Solr
#
