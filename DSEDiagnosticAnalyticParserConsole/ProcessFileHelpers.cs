...
namespace DSEDiagnosticAnalyticParserConsole
{
    static partial class ProcessFileTasks
    {
        ...
        readonly static Regex SSTableCQLTableNameRegEx = new Regex("^(?:([a-z0-9\\-_$%+=@!?<>^*&]+)\\-([0-9a-f]{32})$|([a-z0-9\\-_$%+=@!?<>^*&]+)$)", RegexOptions.Compiled | RegexOptions.IgnoreCase);
        readonly static string[] SSTableVersionMarkers = new string[] { "tmp", "ka", "jb", "ja", "ic" };
            ...
            if (sstableFilePathParts.Length >= 3)
            {
                ...
                if (tblNameMatch.Success)
                {
                                ...
                                && sstableFilePathParts.Last()[ksName.Length + columnFamily.Length + 1] == '.')
                    {
                        ...
                        foreach (var ssVersion in SSTableVersionMarkers)
                        {
